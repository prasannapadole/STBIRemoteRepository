package com.institute.applyforinternship;
...
/**
 * Servlet implementation class InstituteApplyforInternship
 */
//@WebServlet("/InstituteApplyforInternship")
public class InstituteApplyforInternship extends HttpServlet 
    {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
		     ...
		     long instituteappliedId=11111;
		     
		     String query="select max(instituteappliedId) from institute_applied_for_internship";   //select max of id from ideasumit table 
				PreparedStatement pst=con.prepareStatement(query);
				 rs=pst.executeQuery();
				int dbinstituteappliedId=0;
				while(rs.next())
				{
					dbinstituteappliedId=rs.getInt(1);//to get max of id from ideasubmit table
		        }
				if(dbinstituteappliedId==0)
				{
					instituteappliedId=dbinstituteappliedId+instituteappliedId; 
				}
				else
				{
					instituteappliedId=dbinstituteappliedId+1; 
				}
	    
			    String query1="select sigId from sigregi where emailid=?";
			    ps=con.prepareStatement(query1);
			    ps.setString(1,institutormail);
			    r=ps.executeQuery();
			     long dbsigid=0;
			    while(r.next())
			    {
		    ...
		    ps1.setLong(1,instituteappliedId);
