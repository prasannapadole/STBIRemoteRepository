...
/**
 * Servlet implementation class InstituteLogin
 */
//@WebServlet("/InstituteLogin")
public class InstituteLogin extends HttpServlet {
	...
	if(InstituteLoginDao.checkUser(email,password) ) //call LoginDao method that contain email and password code
				        {
					  session.setAttribute("institutormail",email);
			          response.sendRedirect("institute-dashboard.jsp");
				        }
						else
				  		{
