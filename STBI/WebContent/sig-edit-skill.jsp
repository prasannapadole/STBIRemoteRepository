...
<%
long sigtechid=Long.parseLong(request.getParameter("techid"));
String query2="select * from sigtechnologyskill  where techId=?";
pst2=con.prepareStatement(query2);
pst2.setLong(1,sigtechid);
rs2=pst2.executeQuery();
long dbtechId=0;
String dbtitle="",dbleadfaculty="",dbfacultymember="",dbmemberofstudent="",dbdescription="";
while(rs2.next())
    {
	dbtechId=rs2.getLong("techId");
	dbtitle=rs2.getString("title");
	dbleadfaculty=rs2.getString("leadfaculty");
	dbfacultymember=rs2.getString("facultymember");
	dbmemberofstudent=rs2.getString("memberofstudent");
	dbdescription=rs2.getString("description");
	}

%>
	<div style="padding: 30px;">
		<form action="./SigEditSkill" method="post" style="margin-top: 100PX;">
			  <div class="form-group">
			    <label for="inputsm">Title of Technology/ Innovation / Solution:</label>
			    <input class="form-control input-sm" id="inputsm" type="text" name="title" value="<%=dbtitle %>" ng-model="title" required/>
			 <input type="hidden" name="techId" value="<%=dbtechId %>">
			  </div>
			 <div class="form-group">
			    <label for="inputsm">Lead of Faculty :</label>
			    <input class="form-control input-sm" id="inputsm" type="text" name="lead" value="<%=dbleadfaculty %>" ng-model="lead" required/>
			  </div>
			 <div class="form-group">
			    <label for="inputsm">Faculty Member:</label>
			  <input class="form-control input-sm" id="inputsm" type="text" name="member" value="<%=dbfacultymember %>" ng-model="member" required/>
			  </div>
			  <div class="form-group">
			    <label for="inputsm">Add Member of Student:</label>
			   <input class="form-control input-sm" id="inputsm" type="text" name="students" value="<%=dbmemberofstudent %>" ng-model="students" required/> 
			  </div>
			  <div class="form-group">
			    <label for="inputsm">Brief Description:</label> 
			   <textarea rows="" cols="" class="form-control" name="description" ng-model="description" required><%=dbdescription %></textarea>
			  </div>
